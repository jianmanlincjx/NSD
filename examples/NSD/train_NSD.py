...
from examples.brushnet.base_model import AttnProcessor, IPAttnProcessor, make_layers, LinerProjection, SCT
...
def log_validation(
    vae, text_encoder, tokenizer, unet, brushnet, args, accelerator, weight_dtype, step, image_encoder, image_proj_model, is_final_validation=False
):
    ...
    for validation_prompt, validation_image, validation_mask in zip(validation_prompts, validation_images, validation_masks):
        validation_path = validation_image
        ...
        new_size = (validation_image.size[0] // 2, validation_image.size[1] // 2)
        validation_image = validation_image.resize(new_size, Image.ANTIALIAS)
        validation_mask = validation_mask.resize(new_size, Image.ANTIALIAS)

        images = []

        image_temp = cv2.imread(validation_path)
        image_temp = cv2.cvtColor(image_temp, cv2.COLOR_BGR2RGB)
        image_temp = (image_temp / 127.5) - 1.0
        pixel_values = torch.tensor(image_temp).permute(2,0,1).unsqueeze(0).cuda().to(torch.float16)
        style_code = image_proj_model(image_encoder(pixel_values))

        for _ in range(args.num_validation_images):
            with inference_ctx:
                image = pipeline(
                     validation_prompt, validation_image, validation_mask, num_inference_steps=20, generator=generator, style_code=style_code
                ).images[0]
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            for log in image_logs:
                ...
                new_size = (np.asarray(images[0]).shape[1],np.asarray(images[0]).shape[0])
                validation_image = log["validation_image"].resize(new_size, Image.ANTIALIAS)
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--json_file",
        type=str,
        default='/data0/JM/code/lang-segment-anything/train/train.json',
        help=(
...
class MyWebDataset(Dataset):
    def __init__(self, json_file, resolution=None, tokenizers=None, random_mask=None):
        """
        Args:
            json_file (str): Path to the JSON file containing image and text data.
            resolution (tuple): Desired resolution for the images.
            text_encoders (list): List of text encoders.
            tokenizers (PreTrainedTokenizer): Tokenizer for processing text data.
            random_mask (bool): Whether to apply random masking.
        """
        super().__init__()
        ...
        self.vid_list = ['chair', 'sofa', 'ceilings']
    ...
    def random_brush_gen(self, max_tries, h, w, min_num_vertex=0, max_num_vertex=8, mean_angle=2*math.pi/5, angle_range=2*math.pi/15, min_width=128, max_width=128):
        ...
        for _ in range(np.random.randint(1, max_tries + 1)):  # Ensure at least one try
            num_vertex = np.random.randint(min_num_vertex, max_num_vertex + 1)
    ...
    def random_mask_gen(self, h, w):
        ...
        brush_mask = self.random_brush_gen(4, h, w)
        mask = np.logical_and(mask, 1 - brush_mask)  # hole denoted as 0, reserved as 1

        # Ensure the mask is not all white
        if np.all(mask == 1):
            mask = self.random_mask_gen(h, w)  # Regenerate the mask if it is all white
    ...
    def __getitem__(self, idx):
        ...
        mask_file = os.path.join(os.path.dirname(image_file).replace('image', 'mask'), random.choice(self.vid_list), image_name)
        image = cv2.imread(image_file)

        # if not os.path.exists(mask_file):
        #     # mask = self.random_mask_gen(image.shape[0], image.shape[1])[0][:,:,np.newaxis]
        #     caption = 'An elegant and luxurious study room with a large window, a classic desk, a vibrant blue armchair, and a well-organized bookshelf. The room features a sophisticated color palette, with beige walls, dark wood flooring, and decorative elements such as a patterned rug, framed black and white photographs, and modern lighting fixtures. The space exudes a sense of comfort and refinement, perfect for reading, working, or relaxing.'
        #     image_file = "/data1/JM/BrushNet/data/me/image/000001.jpg"
        #     image_name = os.path.basename(image_file)
        #     mask_file = os.path.join(os.path.dirname(image_file).replace('image', 'mask'), random.choice(self.vid_list), image_name)
        #     image = cv2.imread(image_file)
        ...
        # combined_image = np.hstack((image, masked_image))
        # cv2.imwrite('concat_image.png', combined_image)
...
def collate_fn(data):
    pixel_values = torch.stack([example["pixel_values"] for example in data]).to(memory_format=torch.contiguous_format).to(dtype=torch.float16)
    conditioning_pixel_values = torch.stack([example["conditioning_pixel_values"] for example in data]).to(memory_format=torch.contiguous_format).to(dtype=torch.float16)
    masks = torch.stack([example["masks"] for example in data]).to(memory_format=torch.contiguous_format).to(dtype=torch.float16)
    input_ids = torch.stack([example["input_ids"] for example in data]).to(memory_format=torch.contiguous_format)
...
class BrushNetAdapter(torch.nn.Module):
    """IP-Adapter"""
    def __init__(self, unet, brushnet, style_extract, adapter_modules, linproject):
        super().__init__()
        self.unet = unet
        self.style_extract = style_extract
        self.adapter_modules = adapter_modules
        self.brushnet = brushnet
        self.linproject = linproject


    def forward(self, style_images, noisy_latents, timesteps, encoder_hidden_states, conditioning_latents, weight_dtype):

        ip_tokens_temp = self.style_extract(style_images)
        ip_tokens = self.linproject(ip_tokens_temp)
...
def main(args):
    ...
    style_extract = SCT(encoder=vgg)
    style_extract_params = torch.load(args.pretrain_vgg)
    style_extract.load_state_dict(style_extract_params, strict=True)

    vae.requires_grad_(False)
    unet.requires_grad_(False)
    text_encoder.requires_grad_(False)
    brushnet.requires_grad_(False)
    style_extract.requires_grad_(False)
    
    ##################################################################################################
    ##################################################################################################
    ##################################################################################################
    # init adapter modules
    LinerProj = LinerProjection()
    attn_procs = {}
    ...
    brushnet_adapter = BrushNetAdapter(unet=unet, brushnet=brushnet, style_extract=style_extract, adapter_modules=adapter_modules, linproject=LinerProj)
    ##################################################################################################
    ##################################################################################################
    ##################################################################################################
    ...
    style_extract.to(accelerator.device, dtype=weight_dtype)
    ...
    # Optimizer creation
    params_to_opt = itertools.chain(brushnet_adapter.linproject.parameters(),  brushnet_adapter.adapter_modules.parameters())
    ...
    train_dataset = MyWebDataset(json_file = args.json_file, resolution = args.resolution, tokenizers = tokenizer, random_mask=args.random_mask)
    train_dataset_len= 100*len(train_dataset)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(brushnet_adapter):
                ...
                model_pred = brushnet_adapter(batch["pixel_values"].to(dtype=weight_dtype), noisy_latents, timesteps, encoder_hidden_states, conditioning_latents, weight_dtype)
                ...
                if accelerator.is_main_process:
                    ...
                    if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                        image_logs = log_validation(
                            ...
                            weight_dtype,
                            global_step,
                            brushnet_adapter.module.style_extract,
                            brushnet_adapter.module.linproject,
                        )

...
class MetricsCalculator:
    def __init__(self, device,ckpt_path="/data1/JM/code/BrushNet/pretrain_model") -> None:
        ...
        self.SCT.load_state_dict(torch.load(f'{ckpt_path}/model_epoch_20.pth'))
...
vid_list = sorted(os.listdir(args.image_save_path))
vid_list = [vid for vid in vid_list if not vid.endswith('.csv')]

for vid in vid_list:
    # evaluation_df = pd.DataFrame(columns=['Image ID', 'Image Reward', 'HPS V2.1', 'Aesthetic Score', 'PSNR', 'LPIPS', 'MSE', 'CLIP Similarity', 'style_con', 'HP', 'image_path'])
    evaluation_df = pd.DataFrame(columns=['Image ID', 'SS', 'image_path'])

    record_txt = os.path.join(args.data_root, args.data_name, vid, 'record_new.txt')
    data_list = []
    with open(record_txt, 'r') as file:
        for line in file:
            parts = line.strip().split()
            if len(parts) == 2:
                data_dict = {'image': parts[0], 'mask': parts[1]}
                data_list.append(data_dict)
    for key, item in enumerate(data_list):
        src_image_path = os.path.join(args.data_root, args.data_name, item["image"])
        src_image_temp = Image.open(src_image_path)
        new_size = (src_image_temp.size[0] // 2, src_image_temp.size[1] // 2)
        src_image = src_image_temp.resize(new_size, Image.ANTIALIAS)

        mask_path = os.path.join(args.data_root, args.data_name, item['mask'])
        mask_temp = cv2.threshold(cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE), 127, 1, cv2.THRESH_BINARY)[1]
        mask = cv2.resize(mask_temp, (new_size))
        mask_ori = mask.copy()[:, :, np.newaxis]
        mask = 1 - mask[:,:,np.newaxis]


        tgt_image_path=src_image_path.replace('image', 'result')
        ...
        text_path = src_image_path.replace('image', 'text').replace('png', 'txt')
        ...
        tgt_size = tgt_image.size
        src_size = src_image.size

        if src_size != tgt_size:
            src_image = src_image.resize(tgt_size, Image.ANTIALIAS)
            mask = cv2.resize(mask, tgt_size)
            print(f"Resized src_image from {src_size} to {tgt_size}")

        if len(mask.shape) == 2:
            mask = mask[:,:,np.newaxis]

        for metric in evaluation_df.columns.values.tolist()[1:]:
            print(f"evluating metric: {metric}: {src_image_path}")
            ...
            if metric == 'SS':
                metric_result = metrics_calculator.calculate_style_consistent(tgt_image_path, mask_path)

            if metric == 'HP':
                metric_result = metrics_calculator.calculate_human_perception(src_image, tgt_image, mask_ori)

            if metric == 'image_path':
                metric_result = src_image_path
    ...
    averaged_results.to_csv(os.path.join(args.image_save_path, vid, "evaluation_result_sum_SS.csv"))
    evaluation_df.to_csv(os.path.join(args.image_save_path, vid, "evaluation_result_SS.csv"))

    print(f"The generated images and evaluation results is saved in {args.image_save_path}/{vid}")
...
from examples.brushnet.base_model import AttnProcessor, IPAttnProcessor
...
def log_validation(
    vae, text_encoder, tokenizer, unet, brushnet, args, accelerator, weight_dtype, step, image_encoder, clip_process, image_proj_model, is_final_validation=False
):
    ...
    for validation_prompt, validation_image, validation_mask in zip(validation_prompts, validation_images, validation_masks):
        ...
        images = []
        raw_image = Image.open(validation_path)
        clip_image = clip_process(images=raw_image, return_tensors="pt").pixel_values.to(torch.float16)
        with torch.no_grad():
            temp = image_encoder(clip_image.to(accelerator.device, dtype=weight_dtype)).image_embeds
        clip_image_embeds = image_proj_model(temp)

        for _ in range(args.num_validation_images):
            with inference_ctx:
                image = pipeline(
                     validation_prompt, validation_image, validation_mask, num_inference_steps=20, generator=generator, style_code=clip_image_embeds
                ).images[0]
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--json_file",
        type=str,
        default='/data1/JM/BrushNet/data/me/train_samll.json',
        help=(
    ...
    parser.add_argument(
        "--pretrained_model_name_or_path",
        type=str,
        default='/data1/JM/BrushNet/pretrain_model/stable-diffusion-v1-5',
        required=False,
    ...
    parser.add_argument(
        ...
        type=str,
        default='/data1/JM/BrushNet/pretrain_model/segmentation_mask_brushnet_ckpt',
    ...
    parser.add_argument(
        "--image_encoder_path",
        type=str,
        default='/data1/JM/BrushNet/pretrain_model/image_encoder',
        required=False,
...
class MyWebDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_file = os.path.join(os.path.dirname(image_file).replace('image', 'mask'), vid, image_name)
        image = cv2.imread(image_file)

        # if not os.path.exists(mask_file):
        #     # mask = self.random_mask_gen(image.shape[0], image.shape[1])[0][:,:,np.newaxis]
        #     image_file = "/data0/JM/code/BrushNet/data/data_train_small/image/003421.png"
        #     image_name = os.path.basename(image_file)
        #     caption = 'An elegant and modern living room with a large sofa, a unique coffee table, and a mix of furniture styles. The room features a neutral color palette with accents of brown and green, and has a large window with blinds, allowing natural light to fill the space. There are several decorative items, including sculptures, a plant, and framed artwork on the walls. The room exudes a sense of luxury and comfort.'
        #     vid =  random.choice(self.vid_list)
        #     mask_file = os.path.join(os.path.dirname(image_file).replace('image', 'mask'), vid, image_name)
        #     image = cv2.imread(image_file)
        ...
        ##############################################################
        ##############################################################
        # # 假设 image, masked_image 和 mask 已经被定义并且裁剪完成
        # # 转换 mask 从 0-1 到 0-255
        # visual_mask = (mask * 255).astype(np.uint8)
        # # 如果 mask 是单通道的，转换为三通道以匹配其他图像
        # if len(visual_mask.shape) == 2 or visual_mask.shape[2] == 1:
        #     visual_mask = cv2.cvtColor(visual_mask, cv2.COLOR_GRAY2BGR)
        # # 水平拼接图像
        # concatenated_image = np.hstack((image, masked_image, visual_mask))
        # # 保存拼接后的图像
        # cv2.imwrite('/data1/JM/code/BrushNet/concatenated_image.jpg', concatenated_image)
        # import torchvision
        # torchvision.utils.save_image(clip_image, '/data1/JM/code/BrushNet/clip.jpg')
        ##############################################################
        ##############################################################
...
def collate_fn(data):
    ...
    clip_image = torch.cat([example["clip_image"] for example in data], dim=0).to(memory_format=torch.contiguous_format)
...
def main(args):
    ...
    image_logs = None
    clip_process = CLIPImageProcessor()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(brushnet_adapter):
                ...
                with torch.no_grad():
                    image_embeds = image_encoder(batch["clip_image"].to(accelerator.device, dtype=weight_dtype)).image_embeds
                ...
                if accelerator.is_main_process:
                    ...
                    if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                        image_logs = log_validation(
                            ...
                            image_encoder,
                            clip_process,
                            brushnet_adapter.module.image_proj_model,
                        )

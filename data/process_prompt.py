...
path = "/data1/JM/code/BrushNet/pretrain_model/models--OpenGVLab--Mini-InternVL-Chat-4B-V1-5"
...
root = '/data1/JM/code/BrushNet/data/data_test_anchor'
vid_list = sorted(os.listdir(root))[6:]

for vid in vid_list:
    vid_path = os.path.join(root, vid, 'image')
    img_list = sorted(os.listdir(vid_path))
    save_root = vid_path.replace('image', 'text')
    os.makedirs(save_root, exist_ok=True)

    for img in tqdm(img_list):
        try:
            # single-round single-image conversation
            question = "请生成这幅图像的英文prompt用于stablediffusion生成，输出的prompt需要以<prompt>开始并以</prompt>结束"
            img_path = f'{vid_path}/{img}'
            save_txt_path = img_path.replace('image', 'text').replace('.png', '.txt')
